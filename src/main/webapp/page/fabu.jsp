...
action=/page/addHouse enctype="multipart/form-data">
<DIV class=infos>
<TABLE class=field>
  <TBODY>
  <TR>
  ...
  <TR>
    ...
    <TD>
      <SELECT class=text name=typeId>
          <c:forEach var="typename" items="${list}">
...
name=floorage></TD></TR>
    ...
    <TD>
      区：<SELECT class=text id="district_id" name=district_id>
             <OPTION  value="">请选择</OPTION>
              <c:forEach items="${list2}" var="dis">
                <OPTION  value=${dis.id}>${dis.name}</OPTION>
              </c:forEach>
         </SELECT>
        街：<SELECT class=text name=streetId id="street_id"><OPTION  value="">请选择</OPTION></SELECT> </TD></TR><!--
						<tr>
  ...
  <TR>
    ...
    <TD><INPUT id=addtp class=text type=file name=pfile> </TD>
  </TR>
  <TR>
