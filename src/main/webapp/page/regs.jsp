...
<DIV class=box>
<FORM method="post" name="d" action=reg>
<DIV class=infos>
<TABLE class=field>
  <TBODY>
  <TR>
    <TD class=field>用 户 名：</TD>
    <TD><INPUT class=text type=text name=name onblur="cxyh1()" id="yhm">&nbsp;<span id="hhh"></span></TD></TR>
  <TR>
...
<DIV class=buttons>
<INPUT value=立即注册 type=submit id="aaa">
...
<script type="text/javascript">
  function cxyh1() {
    var jcname=$("#yhm").val();
    if (jcname!=""){
    $.post("jcUsername",{"name":jcname},function(data){
      if(data.result==1){
        $("#hhh").html("用户名已存在").css("color","red");
        $("#aaa").attr("disabled",true);
      }else{
        $("#hhh").html("用户名可用").css("color","green");
        $("#aaa").attr("disabled",false);
      }
    },"json");
    }else {
      $("#hhh").html("用户名不能为空").css("color","red");
      $("#aaa").attr("disabled",true);
    }
  }
</script>
